...
- **Admin:** Todo bajo `src/app/admin`: layout con menú lateral (Dashboard, **Finanzas**, Productos, Inventario, Compras, Producción, Clientes, **Usuarios**, Pedidos, Configuración). Dashboard operativo con métricas, resumen financiero y alertas operativas. **Finanzas:** dashboard financiero en `src/app/admin/finanzas`. **Usuarios:** gestión de usuarios (crear empleado, dar acceso cliente, activar/desactivar) en `src/app/admin/usuarios`. Inventario incluye materias primas y, en `/admin/inventario/lotes`, inventario de producto terminado (lotes FIFO). Compras: listado, recepción, pagos, cancelación. **Pedidos:** listado, nuevo, detalle en `src/app/admin/pedidos`. Producción: listado de órdenes, nueva orden, detalle por id con consumos de MP y completar orden. **Configuración:** categorías y subruta **Telegram** en `src/app/admin/configuracion/telegram` para notificaciones.
- **Empleado:** En `src/app/empleado`: layout, **pedidos** (listado y detalle en `[id]`), producción (listado y detalle de órdenes, poner en proceso, agregar consumos, completar orden; reutiliza componentes y actions de admin).
- **Cliente:** En `src/app/cliente`: layout, dashboard; **pedidos** (listado y detalle en `pedidos/[id]`); **precios** en `src/app/cliente/precios/page.tsx`.

Los **componentes reutilizables** están en `src/components`: `ui/` (shadcn, incluye tooltip), `admin/` (admin-sidebar, admin-page-transition, table-skeleton, cards-skeleton, empty-state, botones de eliminar por entidad), `boton-cerrar-sesion.tsx`, y carpetas placeholder para empleado, cliente y shared.
...
- **Clientes:** Listado en `src/app/admin/clientes/page.tsx`; crear/editar en `nuevo/` y `[id]/editar/` con forms; **precios por cliente** en `src/app/admin/clientes/[id]/precios/` (page.tsx, actions.ts, form-agregar-precio.tsx, tarjeta-precio.tsx). Eliminar: `src/components/admin/boton-eliminar-cliente.tsx`.
- **Usuarios (admin):** `src/app/admin/usuarios/page.tsx` y `usuarios-content.tsx`; crear empleado con `form-crear-empleado.tsx`; dar acceso cliente con `form-dar-acceso-cliente.tsx`; activar/desactivar con `boton-toggle-usuario.tsx`; actions en `src/app/admin/usuarios/actions.ts`.
- **Pedidos (admin):** Listado en `src/app/admin/pedidos/page.tsx`; nuevo en `nuevo/` con `form-nuevo-pedido.tsx`; detalle en `[id]/page.tsx` y `detalle-pedido.tsx`; actions en `src/app/admin/pedidos/actions.ts`.
- **Pedidos (empleado):** Listado en `src/app/empleado/pedidos/page.tsx`; detalle en `[id]/page.tsx` con `detalle-pedido-empleado.tsx`; actions en `src/app/empleado/pedidos/[id]/actions.ts`.
- **Pedidos (cliente):** Listado en `src/app/cliente/pedidos/page.tsx`; detalle en `[id]/page.tsx` con `detalle-pedido-cliente.tsx`; actions en `src/app/cliente/pedidos/[id]/actions.ts`.
- **Precios (cliente):** Página en `src/app/cliente/precios/page.tsx` (precios visibles para el cliente).
- **Finanzas:** Dashboard en `src/app/admin/finanzas/page.tsx` y `dashboard-financiero.tsx`.
- **Configuración Telegram:** `src/app/admin/configuracion/telegram/page.tsx`, `actions.ts`, `boton-test-telegram.tsx`; integración en `src/lib/telegram.ts`.
- **API alertas de stock:** `src/app/api/alertas-stock/route.ts`.
...
- **Admin Configuración:** Gestionar categorías de productos (listado, crear inline, editar/eliminar en subruta; no eliminar si tiene productos). Subruta **Telegram:** configuración de notificaciones (page, actions, botón test); integración en `lib/telegram.ts`.
- **Admin Pedidos:** Listado de pedidos; crear nuevo pedido (form); ver detalle de pedido.
- **Admin Usuarios:** Listado de usuarios; crear empleado (perfil + auth); dar acceso a cliente (vincular perfil a customer); activar/desactivar usuario (toggle).
- **Admin Finanzas:** Dashboard financiero (métricas, resumen).
...
- **Empleado Pedidos:** Listado de pedidos; detalle de pedido (detalle-pedido-empleado).
- **Cliente Dashboard:** Página de inicio para rol cliente.
- **Cliente Pedidos:** Listado de pedidos del cliente; detalle de pedido (detalle-pedido-cliente).
- **Cliente Precios:** Página de precios visibles para el cliente.
...
- **Server Actions:** En archivos `actions.ts` dentro del módulo (admin/productos, inventario, clientes, configuracion, compras, produccion, **admin/usuarios**, **admin/pedidos**, **admin/configuracion/telegram**, **admin/clientes/[id]/precios**, (auth)/login). Para listados que deben ignorar RLS usar `createAdminClient()` en la página; para mutaciones siempre `createAdminClient()` en la action.
...
Los nombres exactos de columnas y FKs están en el esquema de Supabase y en `src/types/database.ts`. Precios por cliente pueden usar tablas como `customer_prices` o equivalente; revisar `database.ts` y las actions en `admin/clientes/[id]/precios/`.
...
- **Pedidos:** Admin: listado → nuevo pedido → detalle. Empleado: listado y detalle. Cliente: listado y detalle de sus pedidos.
- **Usuarios:** Crear empleado (perfil + auth); dar acceso cliente (vincular perfil a customer); toggle activo/inactivo.
- **Telegram:** Configuración en admin/configuracion/telegram; envío de notificaciones vía `lib/telegram.ts`.
